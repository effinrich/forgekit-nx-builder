...
function scaffoldNxWorkspace(targetDir: string, appName: string): ScaffoldResult {
  ...
  run('pnpm', ['add', '-D', `nx@${nxVersion}`, `@nx/js@${nxVersion}`, `@nx/react@${nxVersion}`], targetDir);
...
export function registerScaffoldWorkspaceTool(server: McpServer): void {
  server.registerTool(
    ...
    async ({ targetDir, projectName, framework }) => {
      ...
      const result = scaffoldNxWorkspace(targetDir, projectName);
      return {
        content: [
          {
            type: 'text',
