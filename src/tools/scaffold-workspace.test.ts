...
describe('scaffold-workspace tool', () => {
  ...
  test('framework "nextjs" returns a coming-in-v2 response without touching the filesystem', async () => {
    ...
    expect(result.content).toEqual(
      expect.arrayContaining([expect.objectContaining({ type: 'text', text: expect.stringContaining('coming in v2') })]),
    );
    expect(readdirSync(targetDir)).toHaveLength(0);
  }, 15_000);

  test(
    'framework "none" scaffolds a real NX react workspace into an empty temp dir',
    async () => {
      client = await connectedClient();
      const targetDir = makeEmptyTempDir();
      ...
      const appPkg = JSON.parse(
        execFileSync('cat', [join(targetDir, 'apps', 'app', 'package.json')], { encoding: 'utf-8' }),
      );
      expect(appPkg.dependencies).toHaveProperty('zustand');

      // NX itself recognizes the structure.
      const projects = JSON.parse(execFileSync('npx', ['nx', 'show', 'projects'], { cwd: targetDir, encoding: 'utf-8' }));
