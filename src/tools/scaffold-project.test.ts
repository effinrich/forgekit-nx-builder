...
describe('scaffold-project — end-to-end wizard orchestration', () => {
  ...
  test(
    'one call, against a fresh empty target dir, produces a complete working monorepo — generated build/lint/test/storybook build all succeed',
    async () => {
      ...
      const summary = (result.content as Array<{ type: string; text?: string }>)
        .map((c) => c.text ?? '')
        .join('\n');
      ...
      const buildOutput = execFileSync('npx', ['nx', 'run-many', '-t', 'build'], { cwd: targetDir, encoding: 'utf-8' });
      ...
      const testOutput = execFileSync('npx', ['nx', 'run-many', '-t', 'test'], { cwd: targetDir, encoding: 'utf-8' });
