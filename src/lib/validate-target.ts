 ...
 * directory.
 */
export function resolveAppName(targetDir: string, appName: string | undefined): string {
  if (appName !== undefined) {
    ...
    if (!PROJECT_NAME_PATTERN.test(appName)) {
      throw new Error(`appName "${appName}" must be lowercase alphanumeric with optional hyphens (e.g. "my-app").`);
    }
    return appName;
  }
  const appsDir = join(targetDir, 'apps');
  if (!existsSync(appsDir)) {
    throw new Error(`No apps/ directory found under "${targetDir}" — run scaffold-workspace first.`);
  }
  const entries = readdirSync(appsDir, { withFileTypes: true })
    .filter((e) => e.isDirectory() && !e.name.endsWith('-e2e'))
    .map((e) => e.name);
  if (entries.length === 0) {
    throw new Error(`No app found under "${appsDir}" — run scaffold-workspace first.`);
  }
  if (entries.length > 1) {
    throw new Error(
