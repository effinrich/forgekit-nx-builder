...
import type { PackageJsonLike } from './json-file.js';

/**
 * The classname-lint rule's config imports the typescript-eslint parser to
 * parse .tsx synta`x. That config is emitted by add-ui-library (Stage B) but
...
export async function ensureEslintDeps(targetDir: string): Promise<void> {
  ...
  const hasEslintDeps = Boolean(
    rootPkg.devDependencies?.eslint &&
    rootPkg.devDependencies?.['typescript-eslint']
  )
  if (!hasEslintDeps) {
