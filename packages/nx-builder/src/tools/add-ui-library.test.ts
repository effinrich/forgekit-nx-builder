...
describe('add-ui-library tool', () => {
  ...
  it('default path installs Panda CSS + Ark UI into libs/shared/ui and the generated app builds with real CSS output', async () => {
    ...
    const pandaConfig = readFileSync(join(targetDir, 'panda.config.ts'), 'utf8');
    ...
    const buildOutput = execFileSync('npx', ['nx', 'build', 'app'], { cwd: targetDir, encoding: 'utf8' });
  ...
  it('paste theming: CSS-vars and Panda-JSON inputs normalize to the same token output', async () => {
    ...
    await callSlowTool(clientA, {
      name: 'scaffold-workspace',
      arguments: { targetDir: cssVarsDir, projectName: 'app', framework: 'none' },
    });
    await callSlowTool(clientA, {
      name: 'add-ui-library',
      arguments: {
    ...
    await callSlowTool(clientB, {
      name: 'scaffold-workspace',
      arguments: { targetDir: jsonDir, projectName: 'app', framework: 'none' },
    });
    await callSlowTool(clientB, {
      name: 'add-ui-library',
      arguments: {
    ...
    const cssVarsConfig = readFileSync(join(cssVarsDir, 'panda.config.ts'), 'utf8');
    const jsonConfig = readFileSync(join(jsonDir, 'panda.config.ts'), 'utf8');
    expect(cssVarsConfig).toContain('#3b82f6');
    expect(jsonConfig).toContain('#3b82f6');
  }, 900_000);

  it('shadcn/Tailwind opt-in path installs successfully and the classname-lint rule catches a raw classname violation', async () => {
    const client = await connectedClient();
    const targetDir = makeEmptyTempDir();
    ...
    const cleanOutput = execFileSync(
      'npx',
      ['eslint', '--no-config-lookup', '-c', eslintConfigPath, 'src/violation.tsx'],
      {
        cwd: join(targetDir, 'libs', 'shared', 'ui'),
        encoding: 'utf8',
      },
